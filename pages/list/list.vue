				...
				<view class="list-music">
					...
					<view class="list-music-item" v-for="(item,index) in playlist.tracks" :key="index" @tap="handleToDetail(item.id)">
						...
						<view class="list-music-song">
							...
							<view class="">
								<!-- <image v-if="privileges[index].flag > 60 && privileges[index].flag < 70 " src="" mode=""></image> -->
								<!-- <image v-if="privileges[index].maxbr == 999000 "  src="" mode=""></image> -->
		...
		onLoad(options){
			...
			list(options.listId).then(res=>{
				...
				if(res[1].data.code==200){
					...
					this.privileges=res[1].data.priviledges
