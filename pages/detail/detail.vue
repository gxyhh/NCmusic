<template>
	<view class="detail">
		...
		<view class="container" v-show="!isLoading">
			<scroll-view scroll-y="true" @scrolltolower="getAfterComment">
...
<script>
	import { songDetail , songUrl , songLyric , songSimi , songComment ,songAfterComment} from '../../common/api.js';
	...
	export default {
		...
		methods: {
			...
			handleToSimi(songId){
			 ...
			 getAfterComment(){
				 this.limit+=1;
				this.offset=(this.limit-1)*15;
				console.log(this.limit);
				 songAfterComment(this.songid,this.offset).then(res=>{
					console.log('精彩评论',this.limit,this.offset,res);
					if(res[1].data.code==200){
						let data = res[1].data.hotComments
						this.songComment =[...this.songComment,...data]
					}
				})
			}
		}
...
<style scoped>
	.detail-play{ width:580rpx; height:580rpx; background:url(~@/static/changpian.png); background-size:cover; margin:210rpx auto 44rpx auto; position: relative;}
	...
	.detail-play view{ position: absolute; width:170rpx; height:266rpx; position: absolute; left:60rpx; right:0;  margin:auto; top:-170rpx; background:url(~@/static/changzhen.png); background-size:cover;}
